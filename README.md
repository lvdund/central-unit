...
The future architecture will be organized into 4 major modules:

#### 1. Single-threaded RRC Task
- **Sequential processing**: No locking required, single-threaded execution
- **Core orchestrator**: Processes all RRC procedures sequentially
- **State management**: Maintains UE state transitions
- **Event coordination**: Coordinates between protocol tasks

#### 2. Event-driven Architecture
- **ITTI messages**: Inter-Task Interface messages trigger procedures
- **Asynchronous operations**: Procedures wait for responses via ITTI
- **Message queues**: Event queues for each protocol task
- **Non-blocking**: Protocol tasks don't block RRC task

#### 3. Stateful Context Management
- **UE context**: Maintained throughout connection lifecycle
- **DU context**: Tracks DU associations and served cells
- **AMF context**: Manages AMF associations and capabilities
- **CU-UP context**: Tracks CU-UP associations and bearer contexts

#### 4. Protocol Separation
- **F1AP task**: Handles SCTP connections to/from DU
  - Encodes/decodes F1AP messages
  - Manages F1AP associations
  - Routes messages to RRC task via ITTI
- **NGAP task**: Handles SCTP connections to/from AMF
  - Encodes/decodes NGAP messages
  - Manages NGAP associations
  - Routes messages to RRC task via ITTI
- **E1AP task**: Handles SCTP connections to/from CU-UP
  - Encodes/decodes E1AP messages
  - Manages E1AP associations
  - Routes messages to RRC task via ITTI

### Design Principles

- **Single-threaded RRC task**: Sequential processing eliminates locking overhead
- **Event-driven**: ITTI messages trigger procedures asynchronously
- **Stateful**: UE context maintained throughout connection lifecycle
- **Protocol separation**: F1AP/NGAP/E1AP handled by separate tasks
- **Asynchronous**: Procedures wait for responses via ITTI messages

This architecture ensures:
- **Scalability**: Single-threaded RRC task can process millions of events per second
- **Reliability**: No race conditions due to sequential processing
- **Maintainability**: Clear separation of concerns between protocol layers
- **Extensibility**: Easy to add new procedures and protocol support